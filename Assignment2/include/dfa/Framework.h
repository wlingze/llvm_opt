...
protected:
  ...
  int position(const TDomainElem &Elem) {
    typename std::vector<TDomainElem>::iterator iter;
    iter = std::find(Domain.begin(), Domain.end(), Elem);
    if (iter == Domain.end()) {
      return -1;
    } else {
      return std::distance(Domain.begin(), iter);
    }
  }
...
private:
  ...
  getMeetOperands(const BasicBlock &BB) const {
    ...
    for (const auto &sBB : successors(&BB)) {
      const Instruction *pInst = &(sBB->front());
      Operands.push_back(InstDomainValMap.at(pInst));
    }
  ...
  getMeetOperands(const BasicBlock &BB) const {
    ...
    for (const auto &pBB : predecessors(&BB)) {
      const Instruction *pInst = &pBB->back();
      Operands.push_back(InstDomainValMap.at(pInst));
    }
  ...
  DomainVal_t meet(const MeetOperands_t &MeetOperands) const {
    ...
    TMeetOp MeetOp;
    DomainVal_t meetDV = MeetOp.top(Domain.size());
